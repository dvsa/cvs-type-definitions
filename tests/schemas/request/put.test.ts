...
describe('required fields', () => {
	...
	it.each(requiredFields)('should have %s required by the backend', (field) => {
		const schemas: Schema[] = validationSchemas.filter((schema) => schema.includes('put'));
...
describe('fields added on get', () => {
	...
	it.each(fieldsAddedOnGet)('should not have %s added by the backend on put request', (field) => {
		const schemas: Schema[] = validationSchemas.filter((schema) => schema.includes('put'));
		const fields: Partial<Record<Schema, string[]>> = {};
		schemas.forEach((path) => {
			const file = readFile(path);
